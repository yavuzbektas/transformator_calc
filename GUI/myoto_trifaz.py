...
class OtoTrifazwindow(QMainWindow):
    def __init__(self, parent=None):
        super(OtoTrifazwindow, self).__init__(parent)
        self.ui = Ui_MainWindow()
        self.ui.setupUi(self)
        self.setWindowTitle("Trafo Hesaplama Programı V0 - Oto Trafosu Trifaz Hesaplama")
        self.veri_kumeleri()
...
# ======================  IZOLASYONLAR =========================
 ...
 # ================= karkas =====================================
    ...
    def olcu_hesapla(self):
        if self.ui.comboBox_sactipi.currentIndex() == 0:
            self.ui.doubleSpinBox_olcu_a.setValue(self.ui.doubleSpinBox_karkas_en.value() * 5)
        else:
            self.ui.doubleSpinBox_olcu_a.setValue(self.ui.doubleSpinBox_karkas_en.value() * 5+self.ui.doubleSpinBox_nuvebosluk.value()*6)


        self.ui.doubleSpinBox_olcu_b.setValue(
            self.ui.doubleSpinBox_karkas_boy.value() + self.ui.doubleSpinBox_klemens_a_deg.value() + self.ui.doubleSpinBox_ayak_a_deg.value() + 2*self.primer_sarim_yukseklik_toplam )
        self.ui.doubleSpinBox_olcu_c.setValue(
            self.ui.doubleSpinBox_karkas_en.value() * 5 + self.ui.doubleSpinBox_klemens_b_deg.value())
    ...
    def trafoolcu_hesapla(self):
        if self.ui.comboBox_sactipi.currentIndex() == 1:
            ...
            self.ui.doubleSpinBox_trafoolcu_a.setValue(math.ceil(self.ui.doubleSpinBox_karkas_en.value() * 3 + 4 * (
                        self.ui.doubleSpinBox_karkas_en.value() / 2 + self.ui.doubleSpinBox_nuvebosluk.value()))+2)

            self.ui.doubleSpinBox_trafoolcu_c.setValue(math.ceil(
                self.ui.doubleSpinBox_karkas_yukseklik.value() + self.ui.doubleSpinBox_karkas_en.value() * 0.15 + 2* self.ui.doubleSpinBox_karkas_en.value() +2))
            self.ui.doubleSpinBox_sacagirlik.setValue(0)
        else:
            self.ui.doubleSpinBox_trafoolcu_a.setValue(math.ceil(self.ui.doubleSpinBox_karkas_en.value() * 6)+2)
            self.ui.doubleSpinBox_trafoolcu_c.setValue(math.ceil(self.ui.doubleSpinBox_karkas_en.value() * 3 +self.ui.doubleSpinBox_karkas_en.value() * 2 +2))
        ...
        self.ui.doubleSpinBox_trafoolcu_b.setValue(2*self.ui.doubleSpinBox_karkas_en.value()*0.7+self.ui.doubleSpinBox_karkas_boy.value()+2)

        self.ui.doubleSpinBox_trafoolcu_d.setValue(
            self.ui.doubleSpinBox_trafoolcu_a.value() - self.ui.doubleSpinBox_karkas_en.value() )
        self.ui.doubleSpinBox_trafoolcu_e.setValue(
            self.ui.doubleSpinBox_karkas_boy.value() + self.ui.doubleSpinBox_karkas_en.value() *0.7+ 6  )
...
# ========================================================
    def hesap_yap(self,gl,gl2,guc,frekans,gauss,karkas_en,karkas_boy,verim,karkas_yuk,baglanti,sarim,cu_par, cu_yog, al_par, al_yog, dig_par, dig_yog,kademe=1):
        ...
        if gl[9][7].value() > 0.0:
            ...
            self.deger = (hp.akim_hesap_3(guc=guc, gerilim=gl[9][7].value()))
        elif gl[8][7].value() > 0.0 :
            self.deger = (hp.akim_hesap_3(guc=guc, gerilim=gl[8][7].value()))
        elif gl[7][7].value() > 0.0 :
            self.deger = (hp.akim_hesap_3(guc=guc, gerilim=gl[7][7].value()))
        elif gl[6][7].value() > 0.0 :
            self.deger = (hp.akim_hesap_3(guc=guc, gerilim=gl[6][7].value()))
        elif gl[5][7].value() > 0.0 :
            self.deger = (hp.akim_hesap_3(guc=guc, gerilim=gl[5][7].value()))
        elif gl[4][7].value() > 0.0 :
            self.deger = (hp.akim_hesap_3(guc=guc, gerilim=gl[4][7].value()))
        elif gl[3][7].value() > 0.0 :
            self.deger = (hp.akim_hesap_3(guc=guc, gerilim=gl[3][7].value()))
        elif gl[2][7].value() > 0.0 :
            self.deger = (hp.akim_hesap_3(guc=guc, gerilim=gl[2][7].value()))
        elif gl[1][7].value() > 0.0 :
            self.deger = (hp.akim_hesap_3(guc=guc, gerilim=gl[1][7].value()))
        else:
            pass
        for i in range(0,10):
...
# ===========================================================
    ...
    def printout_report(self):
        ...
        printout.izolasyon_karsiligi = self.ui.doubleSpinBox_v_p_1.value() * self.ui.doubleSpinBox_akim_p_1.value()*3/math.sqrt(3)
        ...
        for i in range(0, 10):
            if gl[i][7].value() > 0 and kademe > i:
                ...
                gl[i][5].setValue(hp.kesit_hesap_1(
                    akim=gl[i][0].value(),
                    akim_yogunlugu=hp.akim_yogunlugu_1(
                        tel_turu=gl[i][11].currentText(), cu_par=cu_par, cu_yog=cu_yog, al_par=al_par, al_yog=al_yog,
                        dig_par=dig_par, dig_yog=dig_yog)))
                # cap 1 Hesabı -------------------------
                gl[i][2].setValue(hp.cap_hesap_1(
                    kesit=gl[i][5].value()))
                # Kesit 2 Hesabı -------------------------
