...
def spir_hesap_12(Lg,enduktans,f_man,karkas_man,mpl,Um):
    ...
    try:
       sonuc= math.sqrt((Lg+ (mpl/Um))* (enduktans /1000 ) * 10**8/ (0.4 * math.pi*karkas_man*f_man))
        ...
        sonuc=0
    return sonuc
def spir_hesap_13(gerilim, frekans, gauss, karkas_en, karkas_boy, verim, Kf=4.44):
    # spir= gerilim * * (10 ** 8)) / (Kf * frekans * gauss *  (en * boy / 100))) / (verim / 100)
    try : 
        sonuc =round(((gerilim * (10 ** 8)) / (Kf * frekans * gauss * (karkas_en * karkas_boy / 100)*1.5)) / (verim / 100),2)
    except Exception as error:
        print(error,inspect.currentframe().f_code.co_name)
        sonuc=0
    return sonuc
...
def gobekSacAgirlik_hesap_2(sac_tipi,karkas_en):
    ...
    if sac_tipi=="ei_sac":
        ...
        gobekSac=0
    return gobekSac
def gobekSacAgirlik_hesap_3(sac_tipi,karkas_en,karkas_yuk):
    gobekSac=0
    if sac_tipi=="ei_sac":
        gobekSac=karkas_en*3
    elif sac_tipi=="kesme_sac":    
        gobekSac=karkas_yuk+karkas_en*0.1
    else:
        gobekSac=0
    return gobekSac
...
def Lg_oto_hesap_2(enduktans,karkas_man,sp1,karkas_en,karkas_boy,MPL,Um):
    if enduktans <=0:
        sonuc=0
        return sonuc 
    try : 
...
def trafo_hesap_trifaz_Harmonik( gl, guc, frekans, 
                   gauss, karkas_en, karkas_boy, 
                   karkas_yuk, verim, sarim, 
                   primer_sarim_yukseklik_toplam,
                   cu_par,
        ...
        degerler["Lg_mpl"] = Lg_mpl_hesap_1(
            ...
            karkas_boy=karkas_boy,
            Um=Um)
        degerler["Lg_oto"] = Lg_oto_hesap_2(
            enduktans=degerler["enduktans"], 
            karkas_man=degerler["karkas_man"], 
            sp1=degerler["sp1"], 
            karkas_en=karkas_en, 
            ...
            Um=Um)
        degerler["f_oto"]=f_hesap_1(Lg= degerler["Lg_oto"], karkas_Ac=degerler["karkas_man"], karkas_yuk=karkas_yuk)
        degerler["sp1_oto"]=spir_hesap_12(
            Lg=degerler["Lg_oto"],
            enduktans=degerler["enduktans"],
            f_man= degerler["f_oto"],
            karkas_man=degerler["karkas_man"],
            mpl=degerler["mpl"],
            Um=Um)
        degerler["f_mpl"]=f_hesap_1(Lg=degerler["Lg_mpl"], karkas_Ac=degerler["karkas_man"], karkas_yuk=karkas_yuk)
        ...
        degerler["nuveOlcu_a"],degerler["nuveOlcu_b"],degerler["nuveOlcu_c"]=nuve_olcu_hesapla_4(
            ...
            ayak_a=ayak_a)
        degerler["sac_agirlik"]=sac_agirlik_trifaz(karkas_en=karkas_en,karkas_boy=karkas_boy)
        degerler["kesmeSac_agirlik"]=0
        
        degerler["bosluk"]=0
        # spir 1 Hesabı -------------------------

        gl[0]["spir1"]=degerler["sp1_oto"]
        degerler["sp1_man"] =  spir_hesap_12(
            Lg=Lg_man,
            enduktans=degerler["enduktans"],
            f_man= degerler["f_man"],
            karkas_man=degerler["karkas_man"],
        ...
        if gl[0]["tel_en"] > 0:
            ...
            # kesme sac agirlik -----------------
            if sacTipi==1:
                sactipi_txt="kesme_sac"
                degerler["sac_agirlik"]=0
            else:
                sactipi_txt="ei_sac"
        
            yan_sac_agirlik=yanSacAgirlik_hesap_1(
                sac_tipi=sactipi_txt,
                karkas_en=karkas_en,
                toplam_sarim_Yukseklik = gl[0]["sarim_yukseklik"],
                primer_izolasyon=0,
                nuveBosluk=nuveBosluk
                 )
            gobekSac_agirlik=gobekSacAgirlik_hesap_3(
                sac_tipi=sactipi_txt,
                karkas_en=karkas_en,
                karkas_yuk=karkas_yuk)
            degerler["kesmeSac_agirlik"]=kesmeSacAgirlik_hesap_1(
                sac_tipi=sactipi_txt,
                karkas_en=karkas_en,
                karkas_boy=karkas_boy,
                nuve_bosluk=nuveBosluk,
                gobek_sac = gobekSac_agirlik,
                yan_sac=yan_sac_agirlik,
                sac_yogunluk=sacYogunluk )
            # bosluk   -------------------------
            degerler["bosluk"]=bosluk_hesap_1(karkas_en=karkas_en,
            primer_top_yuk=gl[0]["sarim_yukseklik"],sekonder_top_yuk =0 ,primer_izolasyon=0)
            if sacTipi==0 :
                ...
                degerler["bosluk"] =bosluk_hesap_3(karkas_en=karkas_en,
                primer_top_yuk=gl[0]["sarim_yukseklik"],sekonder_top_yuk=0,primer_izolasyon=0,kesme_sac_bosluk=nuveBosluk)
...
def trafo_hesap_UPS( gl, guc, frekans, 
                   gauss, karkas_en, karkas_boy, 
                   karkas_yuk, verim, sarim, 
                   primer_sarim_yukseklik_toplam,
                   primer_izolasyon,
        ...
        if gl[0]["voltaj"]> 0 :
            ...
            if sarim == "sekonder":
                ...
                if baglanti=="Yıldız":
                    ...
                    gl[0]["akim1"]=akim_hesap_3(
                            guc=guc,
                            gerilim=gl[0]["voltaj"])
                elif baglanti=="Üçgen":
                    gl[0]["akim1"]=akim_hesap_4(
                            guc=guc,
                            gerilim=gl[0]["voltaj"])
                else:
                    print("Akım hesaplanamadı . yanlıs Bağlanı Seçimi",baglanti," +")
            ...
            if gl[0]["tel_en"] > 0:
                ...
                if sarim == "sekonder":
                    ...
                    gl[0]["tel_uzunluk"]=(
                                ...
                                a0 + 8 * primer_sarim_yukseklik_toplam + 8 * primer_izolasyon + 4 *
                                gl[0]["tel_yuk"])
                                + 8 * gl[0]["tel_yuk"] * (gl[0]["kat"] - 1) * (gl[0]["kat"] - 2) / 2
                        )/ 1000 * gl[0]["spirkat"] + (
                                a0 + 8 * primer_sarim_yukseklik_toplam +  4 *
                                gl[0]["tel_yuk"] + 8 * gl[0]["tel_yuk"] *
                                (gl[0]["kat"] - 1)) / 1000 * gl[0]["sonkat_spir"]
